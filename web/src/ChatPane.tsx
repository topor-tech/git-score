...
function MessageBody({
  ...
  while ((match = re.exec(text))) {
    ...
    if (match[1]) {
      ...
      nodes.push(
        <span key={`${match[2]}-${match.index}`} className="cite">
          {match[2]}
        </span>,
      );
    }
...
export default function ChatPane({
  ...
  const suggestions = useMemo(() => {
    return checks
      .filter((c) => (c.score_kind === "binary" ? c.score === 0 : c.score < 0.5))
      .slice(0, 4)
      .map((c) =>
        c.score_kind === "binary" ? `Why did ${c.id} fail?` : `How do I improve ${c.id}?`,
      );
  }, [checks]);
  ...
  return (
    <section className="pane">
      ...
      {!ready ? (
        <p className="placeholder">Chat unlocks when every check has finished. v1 has no overall score.</p>
      ) : (
        <>
