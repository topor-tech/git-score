...
from gitscore_back.config import (
    CONTRIBUTOR_CAP,
    GIT_LOG_CAP,
    LLM_FILE_BYTES,
    OPENAI_API_KEY,
...
from gitscore_back.files import list_dir, read_file_for_llm, search_files
...
Do not compute an overall or weighted score. Talk about individual checks only.
Never invent or change check scores or statuses. If asked to change a score, refuse and say the user can edit the repo and re-run.
If asked for an overall score, say this product does not compute one.

Statuses are PASS, WARN, FAIL, NOT_APPLICABLE, UNKNOWN, or null for observation metrics.
UNKNOWN means missing data (often GitLab API), not a passing control. Observation checks have status null on purpose.
...
def execute_tool(name: str, args: dict, *, root: Path, checks: list[dict]) -> Any:
    ...
    if name == "get_check_evidence":
        ...
        for item in checks:
            if item.get("id") == cid or item.get("check_id") == cid:
                return {
                    "id": cid,
